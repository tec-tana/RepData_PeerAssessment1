...
Above, the scatter plot over time shows that there are not obvious patterns in the time series. On average, the number of steps taken per day is **`r formatC(mean(sum_by_date$sum_steps), format="f", big.mark=",", digits=2)`** steps.
Whereas, the median is **`r formatC(median(sum_by_date$sum_steps), format="f", big.mark=",", digits=2)`** steps. 
...
The bar plot above showing the peak interval being **`r formatC(sum_by_interval[which.max(sum_by_interval$sum_steps),]$interval, format="f", big.mark=",", digits=2)`** interval.
